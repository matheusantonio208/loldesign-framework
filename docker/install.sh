...
# setup
useradd -d /home/vizir -m -s /bin/bash vizir

# nginx setup
sed -i 's,user www-data;,daemon off;,' /etc/nginx/nginx.conf
sed -i 's,/run/nginx.pid,/tmp/nginx.pid,' /etc/nginx/nginx.conf
sed -i 's,worker_connections 768,worker_connections 1024,' /etc/nginx/nginx.conf
mkdir -p /var/cache/nginx
chown -R vizir:0 /var/cache/nginx
chmod -R g+w /var/cache/nginx
chown -R vizir:0 /etc/nginx/conf.d
